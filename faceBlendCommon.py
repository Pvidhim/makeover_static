...
def getLandmarks(faceDetector, landmarkDetector, im, FACE_DOWNSAMPLE_RATIO=1, max_attempts=3):
    """
    Detects facial landmarks in the given image.
    Dynamically adjusts the downsample ratio to handle distant or small faces.
    
    Args:
        faceDetector: dlib face detector instance.
        landmarkDetector: dlib shape predictor for landmark detection.
        im: Input image (numpy array).
        FACE_DOWNSAMPLE_RATIO: Initial downsample ratio for face detection.
        max_attempts: Maximum attempts to adjust downsample ratio for distant faces.
        
    Returns:
        List of landmark points (x, y) or an empty list if no landmarks are detected.
    """
    points = []
    attempt = 0
    ...
    while attempt < max_attempts:
        ...
        if len(faceRects) > 0:
            break  # Faces detected, proceed
        else:
            print(f"No faces detected at downsample ratio {FACE_DOWNSAMPLE_RATIO}. Retrying with a lower ratio...")
            FACE_DOWNSAMPLE_RATIO /= 2  # Decrease the downsample ratio
            attempt += 1
    ...
    for face in faceRects:
        ...
        if area > maxArea:
            ...
            maxRect = [
                max(0, face.left()),
                max(0, face.top()),
                min(imSmall.shape[1], face.right()),
                min(imSmall.shape[0], face.bottom())
            ]

    if maxRect is None:
        print("No valid face rectangle detected!")
        return []

    # Scale the face rectangle back to the original image size
