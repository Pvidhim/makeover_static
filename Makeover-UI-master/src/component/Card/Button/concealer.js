...
const ProcessConcealer = ({ base64Image, onProcessComplete, showShades }) => {
  const [showShadesState, setShowShadesState] = useState(showShades || false);
  const [isProcessing, setIsProcessing] = useState(false);
  const { shades, endpoint } = getShadesAndEndpoint("concealer"); // Fetching shades for concealer
  ...
  return (
    <div>
      ...
      {showShadesState && !isProcessing && (
        <div className="mt-4">
          <p className="text-md py-2">Select a Concealer Shade</p>
...
export default ProcessConcealer;
